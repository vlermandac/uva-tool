...
#ifdef __linux__
    const string szHome = "vlermanda"; // szHome is loginusername in the system
...
void huntless (string name, string range) {
    string cmd, uid, subs_usr_last, tmp;
    Problem problem;
    cmd = curlfunc + " --http1.1 https://uhunt.onlinejudge.org/api/uname2uid/" + \
          name + err;
    ...
    subs_usr_last = system_exec (cmd.c_str() );
    Submission usr_last_subs (subs_usr_last);
    usr_last_subs.showless (problem);
}
class submit {
    private:
...
int main (int argc, char *argv[]) {
    try {
        if (argc == 6 && !strcmp (argv[1], "-hunt") && !strcmp (argv[2], "-u")
        ...
        else if (argc == 2 && ( !strcmp (argv[1], "-veredict") || !strcmp (argv[1], "-v") ) ) {
            huntless ("vlermanda", "1");
        }
            ...
            if(plang == "cpp") plang = "5";
            if(plang == "py") plang = "6";
            if(plang == "java") plang = "2";
            submit sub;
            sub.problem_submit (pnumber, argv[1], plang);
        }
